...
def read_results(paths: list[Path]) -> pd.DataFrame:
    ...
    # Backward compatibility for pre-policy result tables.
    if "query_policy" not in frame:
        frame["query_policy"] = "random"
    return frame


def lineage_budget_auc(
    frame: pd.DataFrame,
    *,
    metric: str,
    contrast_column: str,
    ...
    """Reduce nested target/checkpoint rows to one log-budget AUC per lineage and level."""
    ...
    # Checkpoints/edits are repeated observations nested in a lineage. Equal-weight them within
    # lineage × condition × budget before integrating the query-efficiency curve.
    grouped = (
        subset.groupby(["lineage_id", contrast_column, "query_budget"], as_index=False)["value"]
        .mean()
        .sort_values(["lineage_id", contrast_column, "query_budget"])
    )
        ...
        rows.append(
            {
                ...
                contrast_column: str(level),
                "metric": "log_budget_auc",
                "value": auc,
            }
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Frozen lineage-level query-efficiency aggregation"
    )
    parser.add_argument("results", nargs="+", type=Path)
    parser.add_argument(
        "--contrast",
        choices=("method", "query_policy"),
        default="method",
        help="Field defining the paired contrast.",
    )
    parser.add_argument("--level-a")
    parser.add_argument("--level-b")
    parser.add_argument(
        "--fixed-method",
        default="inverse_posterior_mean",
        help="Method retained when contrasting query policies.",
    )
    parser.add_argument(
        "--fixed-query-policy",
        default="random",
        help="Query policy retained when contrasting methods.",
    )
    ...
    for index, specification in enumerate(args.metric):
        ...
        summary, budgets, incomplete = lineage_budget_auc(
            frame,
            metric=metric,
            contrast_column=args.contrast,
        ...
        analyses.append(
            {
                "metric": metric,
                "direction": direction,
                "estimand": "mean paired difference in log2-query-budget AUC",
                "contrast_field": args.contrast,
                "level_a": level_a,
                "level_b": level_b,
                "fixed": fixed,
                "budget_grid": budgets,
                "estimate_a_advantage": estimate.estimate,
                "ci_lower": estimate.lower,
                "ci_upper": estimate.upper,
                "standard_error": estimate.standard_error,
                "paired_lineages": estimate.n_clusters,
                "incomplete_lineage_conditions": incomplete,
                "p_value": p_value,
                "alternative": args.alternative,
            }
        )
    adjusted = holm_adjust([float(row["p_value"]) for row in analyses])
