...
import numpy as np
import pandas as pd
from behavior2weights.stats.inference import(holm_adjust,paired_cluster_bootstrap,paired_cluster_permutation_test,)
def read_results(paths:list[Path])->pd.DataFrame:
    ...
    return frame
def lineage_budget_auc(frame:pd.DataFrame,*,metric:str,contrast_column:str,levels:tuple[str,str],higher_is_better:bool,)->tuple[pd.DataFrame,list[int],int]:
...
def paired_differences(summary:pd.DataFrame,*,contrast_column:str,level_a:str,level_b:str,)->pd.Series:
...
def main()->None:
    ...
    for index,specification in enumerate(args.metric):
        ...
        summary,budgets,incomplete=lineage_budget_auc(frame,metric=metric,contrast_column=args.contrast,levels=(level_a,level_b),higher_is_better=higher,)
        differences=paired_differences(summary,contrast_column=args.contrast,level_a=level_a,level_b=level_b,)
        estimate=paired_cluster_bootstrap(summary,method_a=level_a,method_b=level_b,metric="log_budget_auc",method_column=args.contrast,resamples=args.resamples,seed=args.seed+index,)
        p_value=paired_cluster_permutation_test(differences,permutations=args.permutations,alternative=args.alternative,seed=args.seed+1_000+index,)
