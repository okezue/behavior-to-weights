...
class BehaviorPropertyClassifier(nn.Module):
    """Set-invariant classifier for architecture, lineage factors, and controlled mechanisms."""
    ...
    def __init__(self, config: PropertyModelConfig) -> None:
        ...
        self.heads = nn.ModuleDict(
            {
                name: nn.Linear(config.encoder.latent_dim, size)
                for name, size in config.property_dims.items()
            }
        )
