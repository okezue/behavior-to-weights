...
class InverseModelConfig:
    """Configuration for the amortized behavior-to-parameter posterior.

    The trace encoder is deliberately set-invariant over query/response pairs: each pair is
    encoded independently, then a learned summary token attends over the unordered set.
    Parameter values are decoded by a coordinate-conditioned hypernetwork.
    """
...
class QueryEncoder(nn.Module):
    def __init__(self, config: InverseModelConfig) -> None:
        ...
        layer = nn.TransformerEncoderLayer(
            d_model=config.trace_width,
            nhead=config.trace_heads,
            dim_feedforward=4 * config.trace_width,
...
class TraceSetEncoder(nn.Module):
    def __init__(self, config: InverseModelConfig) -> None:
        ...
        self.observation_projection = nn.Sequential(
            ...
            nn.Linear(config.observation_dim, config.trace_width),
            nn.GELU(),
            nn.Linear(config.trace_width, config.trace_width),
        )
        self.channel_embedding = nn.Embedding(len(CHANNEL_TO_ID), config.trace_width)
        self.pair_projection = nn.Sequential(
            nn.LayerNorm(3 * config.trace_width),
            nn.Linear(3 * config.trace_width, config.trace_width),
            nn.GELU(),
        )
        ...
        self.to_latent = nn.Sequential(
            nn.LayerNorm(config.trace_width),
            nn.Linear(config.trace_width, config.latent_dim),
        )
    ...
    def forward(
        ...
        *,
        ...
        pair_state = self.pair_projection(
            torch.cat([query_state, observation_state, channel_state], dim=-1)
        )
        ...
        if query_mask is not None:
            summary_valid = torch.ones(
                query_mask.shape[0], 1, dtype=torch.bool, device=query_mask.device
            )
...
class CoordinateDecoder(nn.Module):
    def __init__(self, config: InverseModelConfig) -> None:
        ...
        self.address_projection = nn.Sequential(
        ...
        for _layer_index in range(config.decoder_layers):
            ...
            layers.extend(
                [nn.Linear(input_width, output_width), nn.GELU(), nn.Dropout(config.dropout)]
            )
    ...
    def forward(self, latent: Tensor, descriptors: Mapping[str, Tensor]) -> tuple[Tensor, Tensor]:
        ...
        address = self.address_projection(
            torch.cat(
                [
                    self.tensor_embedding(tensor_id),
                    self.role_embedding(role_id),
                    self.layer_embedding(layer_id),
        ...
        log_variance = (
            ...
            .squeeze(-1)
            .clamp(self.min_log_variance, self.max_log_variance)
        )
        return mean, log_variance


class BehaviorToWeights(nn.Module):
    """Posterior parameter estimator from a set of black-box behavior traces."""
    ...
    def __init__(self, config: InverseModelConfig) -> None:
        ...
        self.property_heads = nn.ModuleDict(
            {
                name: nn.Linear(config.latent_dim, count)
                for name, count in config.property_dims.items()
            }
        )
    ...
    def forward(
        ...
        observations: Tensor,
        channel_ids: Tensor,
        descriptors: Mapping[str, Tensor],
        *,
        query_mask: Tensor | None = None,
        token_mask: Tensor | None = None,
    ) -> BehaviorToWeightsOutput:
        latent = self.encode(
            input_ids,
            observations,
            channel_ids,
        ...
        return {
            "latent": latent,
            "weight_mean": mean,
            "weight_log_variance": log_variance,
    ...
    *,
    ...
    for name, output in logits.items():
        if name not in labels:
            continue
        losses.append(
            F.cross_entropy(output, labels[name].to(output.device), ignore_index=ignore_index)
        )
    if not losses:
