...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self,config:MicroTransformerConfig)->None:
        ...
        mask=torch.triu(torch.ones(config.max_seq_len,config.max_seq_len,dtype=torch.bool),diagonal=1)
...
class MicroTransformer(nn.Module):
    ...
    def _init_weights(module:nn.Module)->None:
        if isinstance(module,(nn.Linear,nn.Embedding)):
            ...
            nn.init.zeros_(module.bias)
    def forward(self,input_ids:Tensor,*,capture:bool=False,return_hidden:bool=False,)->Tensor|tuple[Tensor,Tensor]:
        if input_ids.ndim!=2:
    ...
    @torch.no_grad()
    def generate(self,input_ids:Tensor,max_new_tokens:int,*,temperature:float=1.0,generator:torch.Generator|None=None,)->Tensor:
        if temperature<=0:
    ...
    def save(self,path:str|Path,*,metadata:Mapping[str,str]|None=None)->None:
        ...
        config_metadata={f"config.{key}":str(value)for key,value in dataclasses.asdict(self.config).items()}
        if metadata:
            config_metadata.update({str(key):str(value)for key,value in metadata.items()})
        save_file({name:tensor.detach().cpu().contiguous()for name,tensor in self.state_dict().items()},str(path),metadata=config_metadata,)
    @classmethod
    def load(cls,path:str|Path,config:MicroTransformerConfig,*,device:str|torch.device="cpu",)->MicroTransformer:
        model=cls(config)
