...
ROLE_PATTERNS: tuple[tuple[str, ParameterRole], ...] = (
    ("token_embedding.weight", ParameterRole.TOKEN_EMBEDDING),
    ("position_embedding.weight", ParameterRole.POSITION_EMBEDDING),
    ("q_proj.weight", ParameterRole.Q_PROJ_WEIGHT),
    ("q_proj.bias", ParameterRole.Q_PROJ_BIAS),
    ("k_proj.weight", ParameterRole.K_PROJ_WEIGHT),
    ("k_proj.bias", ParameterRole.K_PROJ_BIAS),
    ("v_proj.weight", ParameterRole.V_PROJ_WEIGHT),
    ("v_proj.bias", ParameterRole.V_PROJ_BIAS),
    ("o_proj.weight", ParameterRole.O_PROJ_WEIGHT),
    ("o_proj.bias", ParameterRole.O_PROJ_BIAS),
    ("mlp.fc1.weight", ParameterRole.MLP_IN_WEIGHT),
    ("mlp.fc1.bias", ParameterRole.MLP_IN_BIAS),
    ("mlp.fc2.weight", ParameterRole.MLP_OUT_WEIGHT),
    ("mlp.fc2.bias", ParameterRole.MLP_OUT_BIAS),
...
class AddressSpace:
    """Addressable, architecture-aware view of a floating-point state dictionary."""
    ...
    def __init__(self, specs: Sequence[TensorSpec]) -> None:
        ...
        self._ends = torch.tensor(
            [spec.offset + spec.length for spec in self.specs], dtype=torch.long
        )
    ...
    def unvectorize(
        ...
        *,
        ...
        if vector.numel() != self.total_parameters:
            raise ValueError(
                f"Expected vector of length {self.total_parameters}, got {vector.numel()}"
            )
    ...
    def sample_indices(
        ...
        *,
        ...
        for spec in self.specs:
            role_to_indices.setdefault(spec.role, []).append(
    ...
    def descriptors(self, indices: Tensor) -> dict[str, Tensor]:
        ...
        for address, tensor_position in zip(
            flat_indices.tolist(), tensor_positions.tolist(), strict=True
        ):
            ...
            layers.append(spec.layer + 1)  # reserve zero for non-layer/global tensors
            continuous.append(
                [
                    local_index / max(spec.length - 1, 1),
                    padded_coords[0] / row_denominator,
                    padded_coords[1] / col_denominator,
                    math.log1p(spec.length) / 20.0,
                    math.log1p(padded_shape[0]) / 12.0,
                    math.log1p(padded_shape[1]) / 12.0,
                    min(spec.rank, 4) / 4.0,
                    address / max(self.total_parameters - 1, 1),
                ]
            )
        original_shape = tuple(indices.shape)
        return {
            "tensor_id": torch.tensor(tensor_ids, dtype=torch.long).reshape(original_shape),
            "role_id": torch.tensor(roles, dtype=torch.long).reshape(original_shape),
            "layer_id": torch.tensor(layers, dtype=torch.long).reshape(original_shape),
            "continuous": torch.tensor(continuous, dtype=torch.float32).reshape(*original_shape, 8),
        }
...
class WeightStandardizer:
    ...
    def _lookup(
        self, role_ids: Tensor, device: torch.device, dtype: torch.dtype
    ) -> tuple[Tensor, Tensor]:
    ...
    def save(self, path: str | Path) -> None:
        ...
        path.write_text(
            json.dumps(
                {"means": self.means, "scales": self.scales, "epsilon": self.epsilon},
                indent=2,
                sort_keys=True,
            )
            + "\n"
    ...
    def load(cls, path: str | Path) -> WeightStandardizer:
        raw = json.loads(Path(path).read_text())
        return cls(
            means={int(key): float(value) for key, value in raw["means"].items()},
            scales={int(key): float(value) for key, value in raw["scales"].items()},
