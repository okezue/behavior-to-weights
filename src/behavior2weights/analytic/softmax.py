...
class LinearSoftmaxExperimentConfig:
    """Factorial Tier-0 study for exact and noisy softmax-system identification."""
    ...
    def __post_init__(self) -> None:
        ...
        if "samples" in self.observation_channels and (
            not self.sample_counts or min(self.sample_counts) <= 0
        ):
...
def canonical_softmax_parameters(weight: Tensor, bias: Tensor) -> Tensor:
    """Return class-vs-reference coefficients, eliminating the exact softmax gauge.

    The final class is the reference. The result has shape ``[input_dim + 1, classes - 1]`` and
    uniquely specifies the conditional distribution within this declared gauge.
    """
...
def probabilities_from_canonical(design: Tensor, beta: Tensor) -> Tensor:
    ...
    reference = torch.zeros(
        design.shape[0], 1, dtype=relative_logits.dtype, device=relative_logits.device
    )
...
def recover_linear_softmax(
    ...
    *,
    ...
    smoothing: float = 0.5,
    ridge: float = 0.0,
) -> LinearSoftmaxRecovery:
    """Recover the identifiable parameter orbit from exact probabilities or class counts."""
    ...
    if design.shape[0] != observations.shape[0] or observations.shape[1] < 2:
        raise ValueError(
            "observations must align with design rows and contain at least two classes"
        )
    ...
    if ridge > 0:
        ...
        recovered = torch.linalg.solve(
            gram + ridge * torch.eye(gram.shape[0], dtype=gram.dtype), rhs
        )
    else:
    ...
    return LinearSoftmaxRecovery(
        recovered_beta=recovered,
        design_rank=rank,
        design_condition_number=condition,
        observable_dimension=rank * (classes - 1),
        orbit_dimension=design.shape[1] * (classes - 1),
    )
    ...
    *,
    ...
    if strategy == "gaussian":
        ...
        if query_count <= len(anchors):
            ...
            extra = (
                torch.randn(query_count - len(anchors), input_dim, generator=generator)
                * query_scale
            )
...
def run_linear_softmax_experiment(
    config: LinearSoftmaxExperimentConfig,
    output_directory: str | Path,
) -> dict[str, Any]:
    """Run the complete Tier-0 factorial experiment and write tidy system-level rows."""
            ...
            for query_count in config.query_counts:
                for strategy in config.query_strategies:
                    for channel in config.observation_channels:
                        sample_grid: tuple[int | None, ...] = (
                            tuple(config.sample_counts) if channel == "samples" else (None,)
                        )
                        for sample_count in sample_grid:
                            for replicate in range(config.systems_per_cell):
                                ...
                                weight = (
                                    torch.randn(class_count, input_dim, generator=generator)
                                    * config.weight_scale
                                )
                                bias = (
                                    torch.randn(class_count, generator=generator)
                                    * config.weight_scale
                                )
                                ...
                                if channel == "probabilities":
                                    ...
                                    counts = np.stack(
                                        [
                                            numpy_rng.multinomial(sample_count, row.cpu().numpy())
                                            for row in probabilities
                                        ]
                                    )
                                    observations = torch.from_numpy(counts)
                                recovery = recover_linear_softmax(
                                    design,
                                    observations,
                                    observation_channel=channel,
                                    sample_count=sample_count,
                                    smoothing=config.dirichlet_smoothing,
                                    ridge=config.ridge,
                                )
                                holdout = torch.cat(
                                    [
                                        ...
                                        * config.query_scale,
                                        torch.ones(config.holdout_queries, 1),
                                    ],
                                    dim=1,
                                ).double()
                                true_probabilities = probabilities_from_canonical(holdout, beta)
                                recovered_probabilities = probabilities_from_canonical(
                                    holdout, recovery.recovered_beta
                                )
                                rank_fraction = (
                                    recovery.observable_dimension / recovery.orbit_dimension
                                )
                                rows.append(
                                    {
                                        "system_id": f"softmax-{stable_hash(factors, length=24)}",
                                        **factors,
                                        "seed": seed,
                                        "design_rank": recovery.design_rank,
                                        "design_columns": input_dim + 1,
                                        "design_condition_number": (
                                            recovery.design_condition_number
                                            if math.isfinite(recovery.design_condition_number)
                                            else None
                                        ),
                                        "observable_dimension": recovery.observable_dimension,
                                        "orbit_dimension": recovery.orbit_dimension,
                                        "observable_fraction": rank_fraction,
                                        "orbit_nrmse": _normalized_rmse(
                                            recovery.recovered_beta, beta
                                        ),
                                        "functional_forward_kl": _forward_kl(
                                            true_probabilities,
                                            recovered_probabilities,
                                        ),
                                        "full_rank": recovery.design_rank == input_dim + 1,
                                        "exact_recovery": (
                                            channel == "probabilities"
                                            and recovery.design_rank == input_dim + 1
                                            and _normalized_rmse(recovery.recovered_beta, beta)
                                            < 1e-8
                                        ),
                                    }
                                )
        ...
        cell_summaries.append(
            {
                **dict(zip(keys, cell, strict=True)),
                "systems": len(values),
                "full_rank_rate": float(np.mean([row["full_rank"] for row in values])),
                "exact_recovery_rate": float(np.mean([row["exact_recovery"] for row in values])),
                "mean_orbit_nrmse": float(np.mean([row["orbit_nrmse"] for row in values])),
                "mean_functional_forward_kl": float(
                    np.mean([row["functional_forward_kl"] for row in values])
                ),
                "mean_observable_fraction": float(
                    np.mean([row["observable_fraction"] for row in values])
                ),
            }
        )
    summary = {
        ...
        "interpretation": (
            "Full-probability recovery is exact only when the augmented query design has full "
            "column rank; sampled outputs contract toward the same orbit as repetitions grow."
        ),
    }
