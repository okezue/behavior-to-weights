...
class TracePropertyCorpus:
    def __init__(
        ...
        *,
        ...
        vocabulary: PropertyVocabulary | None = None,
    ) -> None:
        ...
        self.traces = traces
        self.trace_indices = torch.tensor(
            [trace_index[record.target_id] for record in self.records]
        )
    ...
    def batch(
        ...
        *,
        ...
        channel_ids = torch.full(
            (len(rows), len(query_indices)),
            CHANNEL_TO_ID[self.traces.channel],
            dtype=torch.long,
        )
        labels = {
            name: torch.tensor(
                [self.vocabulary.encode(self.records[index], name) for index in target_indices],
                dtype=torch.long,
            )
        ...
        return {
            ...
            "query_mask": torch.ones(len(rows), len(query_indices), dtype=torch.bool),
            "labels": labels,
        }
...
def save_property_checkpoint(
    model: BehaviorPropertyClassifier,
...
def load_property_checkpoint(
    directory: str | Path, *, device: str = "cpu"
) -> tuple[BehaviorPropertyClassifier, PropertyVocabulary]:
    directory = Path(directory)
    config = PropertyModelConfig.from_dict(
...
def train_property_classifier(
    ...
    corpus: TracePropertyCorpus,
    config: PropertyTrainingConfig,
    ...
    *,
    ...
    validation_indices = corpus.indices_for_split(Split.VALIDATION) or corpus.indices_for_split(
        Split.TEST
    )
        ...
        tracker.set_params(
            {
                ...
                "property_vocabulary": corpus.vocabulary.values,
                "train_targets": len(train_indices),
                "validation_targets": len(validation_indices),
                "channel": corpus.traces.channel.value,
            }
        )
    ...
    try:
        for step in range(config.steps):
            ...
            row = {
                ...
                "learning_rate": float(config.learning_rate * scale),
            }
            history.append(row)
            if tracker:
                tracker.track(
                    row["loss"], name="property_loss", step=step, context={"subset": "train"}
                )
                tracker.track(
                    row["learning_rate"],
                    name="learning_rate",
            ...
            with torch.no_grad():
                for repeat in range(config.validation_batches):
                    ...
                    chosen = [
                        validation_indices[int(value)]
                        for value in torch.randint(
                            len(validation_indices),
                            (min(config.batch_size, len(validation_indices)),),
                            generator=qgen,
                        )
                    ]
                    q = torch.randperm(len(corpus.traces.query_ids), generator=qgen)[
                        : max(config.query_budgets)
                    ]
                    ...
                    validation_logits = model(
                        ...
                        validation_batch["channel_ids"],
                        query_mask=validation_batch["query_mask"],
                    )
                    losses.append(
                        float(_loss(validation_logits, validation_batch["labels"]).item())
                    )
        ...
        if not best_dir.exists():
            save_property_checkpoint(
                ...
                corpus.vocabulary,
                best_dir,
                {"validation_loss": best_loss, "step": len(history) - 1},
            )
        (output_directory / "training_history.jsonl").write_text(
