...
class InverseTrainingCorpus:
    """In-memory corpus for one architectural family.

    For large checkpoints, replace this class with the same interface backed by memory-mapped
    safetensors and coordinate-level reads. The experiment protocol intentionally keeps the
    architectural-family boundary explicit for coordinate-level recovery.
    """
    ...
    def __init__(
        ...
        *,
        ...
        selected = [
            ...
            if record.target_id in trace_index
        ...
        if len(architecture_ids) != 1:
            raise ValueError(
                "coordinate-level recovery requires one architecture_id per corpus; "
                "train a pooled property classifier separately"
            )
        ...
        for record in selected:
            ...
            elif [spec.name for spec in current_space.specs] != [
                spec.name for spec in address_space.specs
            ] or [spec.shape for spec in current_space.specs] != [
                spec.shape for spec in address_space.specs
            ]:
                raise ValueError("state dictionaries are incompatible within architecture_id")
            self.targets.append(
                TargetWeights(
                    record=record, state_dict=state, vector=current_space.vectorize(state)
                )
            )
        ...
        self._record_index = {
            target.record.target_id: index for index, target in enumerate(self.targets)
        }
    ...
    def sample_batch(
        ...
        *,
        ...
        fixed_query_indices: Tensor | None = None,
        ...
        if fixed_query_indices is None:
            ...
            query_indices = torch.stack(
                [
                    torch.randperm(len(self.traces.query_ids), generator=generator)[:query_budget]
        ...
        observations = torch.stack(
            [
                self.traces.observations[row, queries]
                for row, queries in zip(trace_rows.tolist(), query_indices, strict=True)
            ]
        )
        ...
        coordinate_indices = self.address_space.sample_indices(
            coordinate_count,
            generator=generator,
            stratified_by_role=True,
        )
        descriptors = self.address_space.descriptors(coordinate_indices)
        role_ids = descriptors["role_id"]
        targets = torch.stack(
            [self.targets[index].vector[coordinate_indices] for index in selected_indices]
        )
        ...
        return {
            ...
            "query_mask": torch.ones(batch_size, query_budget, dtype=torch.bool),
            "descriptors": descriptors,
            "target_weights": standardized_targets,
            "coordinate_indices": coordinate_indices,
            "target_indices": torch.tensor(selected_indices),
        }
