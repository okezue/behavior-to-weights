...
def behavior_to_weight_jacobian(
    ...
    *,
    ...
    """Jacobian of predicted parameter coordinates with respect to trace observations.

    Intended for small diagnostic batches. Large studies should use vector-Jacobian products or
    integrated gradients to avoid materializing [coordinates, queries, observation_features].
    """
    ...
    return cast(
        Tensor,
        torch.autograd.functional.jacobian(prediction, observations, vectorize=True).squeeze(1),
    )


@torch.no_grad()
def transcript_patch_effect(
    model: BehaviorToWeights,
    input_ids_a: Tensor,
    observations_a: Tensor,
    input_ids_b: Tensor,
    observations_b: Tensor,
    channel_ids: Tensor,
    descriptors: Mapping[str, Tensor],
    *,
...
def integrated_gradients_observations(
    model: BehaviorToWeights,
    input_ids: Tensor,
    observations: Tensor,
    channel_ids: Tensor,
