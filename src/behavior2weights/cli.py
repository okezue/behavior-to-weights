...
from behavior2weights.train.property import (
    PropertyTrainingConfig,
    TracePropertyCorpus,
...
def _training_tracker(
    tracking_config: Path | None,
    output: Path,
    *,
    experiment: str,
    run_name: str,
...
def analytic_softmax(
    config: Annotated[Path, typer.Option(exists=True, readable=True)],
    output: Annotated[Path, typer.Option()],
) -> None:
    """Run the Tier-0 linear-softmax identifiability study."""
    raw = load_yaml(config)
    summary = run_linear_softmax_experiment(
        LinearSoftmaxExperimentConfig.from_dict(raw.get("analytic", raw)), output
    )
    typer.echo(
        json.dumps(
            {
...
@app.command()
def doctor() -> None:
    """Print the resolved runtime and optional dependency status."""
...
@app.command()
def smoke(
    output: Annotated[Path, typer.Option(help="Output directory.")] = Path("artifacts/smoke"),
    overwrite: Annotated[bool, typer.Option(help="Remove previous output.")] = False,
) -> None:
    """Run a CPU-sized end-to-end validation experiment."""
...
def plan_zoo(
    config: Annotated[Path, typer.Option(exists=True, readable=True)],
    kind: Annotated[str, typer.Option(help="auto, micro, text, or public")] = "auto",
    output: Annotated[Path | None, typer.Option()] = None,
) -> None:
    """Expand a zoo config into exact counts without training or downloading models."""
...
def build_text(
    config: Annotated[Path, typer.Option(exists=True, readable=True)],
    output: Annotated[Path, typer.Option()],
    dataset: Annotated[Path | None, typer.Option(exists=True)] = None,
    device: Annotated[str, typer.Option()] = "cuda",
) -> None:
    """Train a multi-lineage transparent LM zoo on a prepared text corpus."""
    raw = load_yaml(config)
    dataset_directory = dataset or Path(raw["dataset_directory"])
    policy = SplitPolicy(**raw.get("split_policy", {}))
    result = build_text_zoo(
        TextZooConfig.from_dict(raw.get("zoo", raw)),
        dataset_directory,
        output,
        device=device,
        split_policy=policy,
...
def build_public(
    ...
    """Expand a pinned public-model specification into a target manifest."""
    raw = load_yaml(config)
...
def build_hf_probes(
    prompts: Annotated[Path, typer.Option(exists=True, readable=True)],
    output: Annotated[Path, typer.Option()],
    tokenizer: Annotated[str, typer.Option(help="Pinned tokenizer name or local path.")],
    revision: Annotated[str | None, typer.Option()] = None,
    sequence_length: Annotated[int, typer.Option(min=2)] = 128,
    text_field: Annotated[str, typer.Option()] = "text",
    local_files_only: Annotated[bool, typer.Option()] = False,
) -> None:
    """Tokenize a JSONL prompt suite into a fixed-length reusable query bank."""
    records = build_hf_query_bank(
        prompts,
        output,
...
def collect_micro_traces(
    manifest: Annotated[Path, typer.Option(exists=True)],
    queries: Annotated[Path, typer.Option(exists=True)],
    output: Annotated[Path, typer.Option()],
    channel: Annotated[ObservationChannel, typer.Option()] = ObservationChannel.LOGITS,
    feature_dim: Annotated[int | None, typer.Option(min=1)] = None,
    topk: Annotated[int, typer.Option(min=1)] = 8,
    sample_count: Annotated[int, typer.Option(min=1)] = 32,
    temperature: Annotated[float, typer.Option(min=0.000001)] = 1.0,
    sketch_dim: Annotated[int, typer.Option(min=1)] = 16,
    sketch_seed: Annotated[int, typer.Option()] = 17,
    center_logits: Annotated[bool, typer.Option()] = True,
    batch_size: Annotated[int, typer.Option(min=1)] = 128,
    base_seed: Annotated[int, typer.Option()] = 0,
    device: Annotated[str, typer.Option()] = "cpu",
) -> None:
    ...
    default_feature_dim = (
        ...
        else (2 * min(topk, vocab_size) if channel == ObservationChannel.TOPK else vocab_size)
    )
    observation = ObservationConfig(
        channel=channel,
        vocab_size=vocab_size,
        feature_dim=feature_dim or default_feature_dim,
    ...
    bundle = collect_target_traces(
        records,
        query_records,
        adapter.load,
        observation,
        CollectionConfig(batch_size=batch_size, device=device, base_seed=base_seed),
...
def collect_micro_trace_suite(
    ...
    """Collect every channel declared in an experiment YAML into immutable sub-stores."""
    records = load_manifest(manifest, resolve_paths=False)
    query_records = [QueryRecord.model_validate(row) for row in read_jsonl(queries)]
    vocab_sizes = {int(record.metadata["model_config"]["vocab_size"]) for record in records}
    if len(vocab_sizes) != 1:
    ...
    for raw_spec in channel_specs:
        ...
        bundle = collect_target_traces(
            ...
            observation,
            CollectionConfig(batch_size=batch_size, device=device, base_seed=base_seed),
            output_directory=channel_output,
        )
        suite_rows.append(
            {
                "channel": observation.channel.value,
                "directory": str(channel_output),
                "shape": list(bundle.observations.shape),
                "observation_config": dataclasses.asdict(observation),
...
def collect_hf_traces(
    manifest: Annotated[Path, typer.Option(exists=True)],
    queries: Annotated[Path, typer.Option(exists=True)],
    output: Annotated[Path, typer.Option()],
    external_family: Annotated[str | None, typer.Option()] = None,
    architecture_id: Annotated[str | None, typer.Option()] = None,
    channel: Annotated[ObservationChannel, typer.Option()] = ObservationChannel.LOGITS,
    vocab_size: Annotated[int, typer.Option(min=1)] = 50_257,
    feature_dim: Annotated[int | None, typer.Option(min=1)] = None,
    topk: Annotated[int, typer.Option(min=1)] = 32,
    sample_count: Annotated[int, typer.Option(min=1)] = 64,
    temperature: Annotated[float, typer.Option(min=0.000001)] = 1.0,
    sketch_dim: Annotated[int, typer.Option(min=1)] = 512,
    sketch_seed: Annotated[int, typer.Option()] = 17,
    center_logits: Annotated[bool, typer.Option()] = True,
    base_seed: Annotated[int, typer.Option()] = 0,
    ...
    device: Annotated[str, typer.Option()] = "cuda",
    local_files_only: Annotated[bool, typer.Option()] = False,
    fail_fast: Annotated[bool, typer.Option()] = True,
) -> None:
    """Collect traces from a homogeneous Hugging Face model family."""
    records = load_manifest(manifest, resolve_paths=False)
    records = [
        record
        for record in records
        if (external_family is None or record.factors.get("external_family") == external_family)
        and (architecture_id is None or record.architecture_id == architecture_id)
    ]
    if not records:
    ...
    if len(tokenizer_names) != 1:
        raise typer.BadParameter(
            "selected targets use multiple tokenizers; collect each tokenizer family separately"
        )
    query_records = [QueryRecord.model_validate(row) for row in read_jsonl(queries)]
    adapter = HuggingFaceCausalLMAdapter(
        manifest_root=manifest.parent, local_files_only=local_files_only
    )
    bundle = collect_target_traces(
        records,
        query_records,
        ...
        ObservationConfig(
            ...
            or (
                ...
                if channel == ObservationChannel.LOGIT_SKETCH
                    ...
                    2 * min(topk, vocab_size)
                    if channel == ObservationChannel.TOPK
                    else (
                        min(vocab_size, 512)
                        if channel
                        in {ObservationChannel.TOKENS, ObservationChannel.SAMPLE_HISTOGRAM}
                        else vocab_size
                    )
                )
            ),
...
def train_inverse(
    manifest: Annotated[Path, typer.Option(exists=True)],
    traces: Annotated[Path, typer.Option(exists=True)],
    config: Annotated[Path, typer.Option(exists=True)],
    output: Annotated[Path, typer.Option()],
    architecture_id: Annotated[str | None, typer.Option()] = None,
    tracking_config: Annotated[Path | None, typer.Option("--tracking-config", exists=True)] = None,
    run_name: Annotated[str, typer.Option()] = "inverse",
    ...
    corpus = InverseTrainingCorpus(
        ...
        canonicalize=bool(raw.get("canonicalize", True)),
    )
    ...
    tracker = _training_tracker(
        tracking_config, output, experiment="behavior-to-weights/inverse", run_name=run_name
    )
    result = train_inverse_model(
        BehaviorToWeights(model_config), corpus, training_config, output, tracker=tracker
    )
...
def train_properties(
    ...
    output: Annotated[Path, typer.Option()],
    tracking_config: Annotated[Path | None, typer.Option("--tracking-config", exists=True)] = None,
    run_name: Annotated[str, typer.Option()] = "properties",
    seed: Annotated[int | None, typer.Option()] = None,
    device: Annotated[str | None, typer.Option()] = None,
) -> None:
    """Train pooled architecture/training/circuit property classifiers."""
    raw = load_yaml(config)
    records = load_manifest(manifest, resolve_paths=False)
    bundle = load_trace_bundle(traces)
    ...
    tracker = _training_tracker(
        tracking_config, output, experiment="behavior-to-weights/properties", run_name=run_name
    )
    result = train_property_classifier(
        PropertyModelConfig(encoder=encoder, property_dims=corpus.vocabulary.dimensions),
        corpus,
        training,
        output,
        tracker=tracker,
...
def train_tokenizer(
    ...
    """Train a byte-level BPE tokenizer for custom small-language-model zoos."""
    raw = load_yaml(config)
...
def download_nfn(
    name: Annotated[str, typer.Option(help="mnist or ag_news")],
    output: Annotated[Path, typer.Option()],
...
def evaluate_micro(
    manifest: Annotated[Path, typer.Option(exists=True)],
    traces: Annotated[Path, typer.Option(exists=True)],
    checkpoint: Annotated[Path, typer.Option(exists=True)],
    output: Annotated[Path, typer.Option()],
    architecture_id: Annotated[str | None, typer.Option()] = None,
    budgets: Annotated[str, typer.Option(help="Comma-separated query budgets.")] = "8,16,32,64",
    query_policies: Annotated[
        str, typer.Option(help="Comma-separated random,population_disagreement policies.")
    ] = "random,population_disagreement",
    splits: Annotated[str, typer.Option(help="Comma-separated test,ood splits.")] = "test,ood",
    functional_examples: Annotated[int, typer.Option(min=1)] = 256,
    tier: Annotated[str, typer.Option()] = "tier1",
    run_id: Annotated[str, typer.Option()] = "evaluation",
    replicate: Annotated[int, typer.Option(min=0)] = 0,
    seed: Annotated[int, typer.Option()] = 20260621,
    device: Annotated[str, typer.Option()] = "cpu",
) -> None:
    """Evaluate reconstruction, calibration, baselines, and edit localization."""
    ...
    rows = evaluate_micro_inverse(
        ...
        config=EvaluationConfig(
            query_budgets=parsed_budgets,
            query_policies=parsed_policies,
            splits=parsed_splits,
            functional_examples=functional_examples,
            tier=tier,
            run_id=run_id,
            replicate=replicate,
            seed=seed,
...
def evaluate_properties(
    ...
    checkpoint: Annotated[Path, typer.Option(exists=True)],
    output: Annotated[Path, typer.Option()],
    budgets: Annotated[str, typer.Option()] = "16,32,64",
    query_policies: Annotated[str, typer.Option()] = "random,population_disagreement",
    splits: Annotated[str, typer.Option()] = "test,ood",
    tier: Annotated[str, typer.Option()] = "tier2",
    run_id: Annotated[str, typer.Option()] = "property-evaluation",
    replicate: Annotated[int, typer.Option(min=0)] = 0,
    seed: Annotated[int, typer.Option()] = 20260621,
    device: Annotated[str, typer.Option()] = "cpu",
) -> None:
    ...
    parsed_policies = tuple(value.strip() for value in query_policies.split(",") if value.strip())
    parsed_splits = tuple(Split(value.strip()) for value in splits.split(",") if value.strip())
    rows = evaluate_property_classifier(
        manifest_path=manifest,
        traces_directory=traces,
        checkpoint_directory=checkpoint,
        output_path=output,
...
def power(
    effect: Annotated[float, typer.Option(help="Standardized paired effect size.")],
    icc: Annotated[float, typer.Option()] = 0.5,
    checkpoints: Annotated[int, typer.Option()] = 1,
    lineages: Annotated[int, typer.Option()] = 80,
    simulations: Annotated[int, typer.Option()] = 5_000,
) -> None:
    result = simulate_clustered_power(
        lineages=lineages,
        checkpoints_per_lineage=checkpoints,
