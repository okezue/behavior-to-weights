...
class ObservationConfig:
    ...
    def from_dict(
        ...
        *,
        ...
        return cls(channel=channel, vocab_size=vocab_size, **data)  # type: ignore[arg-type]
    ...
    def __post_init__(self) -> None:
        ...
        if self.channel == ObservationChannel.LOGITS and self.vocab_size > self.feature_dim:
            raise ValueError(
                "full-logit observations require feature_dim >= vocab_size; use logit_sketch "
...
def encode_logits(
    ...
    *,
    ...
    """Convert final-position logits to a fixed-width black-box observation.

    Returned metadata includes sampled token IDs or top-k arrays when applicable. The fixed-width
    representation is what the inverse model consumes; metadata preserves the exact API-like trace.
    """

    if logits.ndim != 2 or logits.shape[-1] != config.vocab_size:
        raise ValueError(
            f"logits must have shape [batch, {config.vocab_size}], got {tuple(logits.shape)}"
        )
    ...
    if config.channel == ObservationChannel.LOGITS:
        ...
        observation.scatter_add_(
            1,
            buckets,
            torch.ones_like(buckets, dtype=observation.dtype),
        )
...
def collect_observations(
    ...
    *,
    ...
    seed: int = 0,
    row_seeds: Sequence[int] | Tensor | None = None,
    logits_selector: Callable[[Tensor], Tensor] | None = None,
) -> tuple[Tensor, dict[str, Tensor]]:
    if input_ids.ndim != 2:
    ...
    for start in range(0, input_ids.shape[0], batch_size):
        ...
        if not isinstance(output, Tensor):
            raise TypeError(
                "model output must be a Tensor, tuple containing a Tensor, or have .logits"
            )
        ...
        if row_seeds is None:
            ...
            for local_index in range(logits.shape[0]):
                row_generator = torch.Generator(device=generator_device).manual_seed(
                    row_seeds[start + local_index]
                )
                row_encoded, row_metadata = encode_logits(
    ...
    return torch.cat(observations), {
        key: torch.cat(values) for key, values in metadata_accumulator.items()
    }
