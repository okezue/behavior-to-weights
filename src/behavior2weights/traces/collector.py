...
def _sampling_seed(
    *,
    ...
    payload = {
        "scheme": "behavior2weights-trace-rng-v1",
...
def collect_target_traces(
    ...
    *,
    ...
    stochastic = observation_config.channel in {
        ObservationChannel.TOKENS,
        ObservationChannel.SAMPLE_HISTOGRAM,
    }
    for target in targets:
        try:
            ...
            row_seeds = (
                [
                    _sampling_seed(
                        base_seed=collection_config.base_seed,
            ...
            observations, auxiliary = collect_observations(
                model,
                input_ids,
                observation_config,
                batch_size=collection_config.batch_size,
                device=collection_config.device,
                seed=collection_config.base_seed,
                row_seeds=row_seeds,
            )
        ...
        except Exception as error:  # pragma: no cover - exercised on remote model failures
    ...
    bundle = TraceBundle(
        ...
        input_ids=input_ids,
        observations=torch.stack(trace_tensors),
        channel=observation_config.channel,
        feature_dim=observation_config.feature_dim,
        auxiliary={key: torch.stack(values) for key, values in auxiliary_tensors.items()},
        metadata={
            "observation_config": dataclasses.asdict(observation_config),
            "collection_config": dataclasses.asdict(collection_config),
            "failures": failures,
            "rng_scheme": (
                "sha256(base_seed,target_id,query_id,channel)-v1"
                if stochastic
                else "deterministic-channel"
            ),
        },
    )
