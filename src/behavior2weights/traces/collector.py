...
from behavior2weights.traces.observations import ObservationConfig,collectobservations
from behavior2weights.traces.store import TraceBundle,savetracebundle
...
def samplingseed(*,base_seed:int,target_id:str,query_id:str,channel:ObservationChannel,)->int:
    ...
    return int(stablehash(payload,length=16),16)%(2**63-1)
def collecttargettraces(targets:Sequence[TargetRecord],queries:Sequence[QueryRecord],model_loader:ModelLoader,observation_config:ObservationConfig,collection_config:CollectionConfig,*,output_directory:str|Path|None=None,)->TraceBundle:
    if not targets:
    ...
    for target in targets:
        try:
            ...
            row_seeds=([samplingseed(base_seed=collection_config.base_seed,target_id=target.target_id,query_id=query.query_id,channel=observation_config.channel,)for query in queries]if stochastic else None)
            observations,auxiliary=collectobservations(model,input_ids,observation_config,batch_size=collection_config.batch_size,device=collection_config.device,seed=collection_config.base_seed,row_seeds=row_seeds,)
    ...
    if output_directory is not None:
        savetracebundle(bundle,output_directory)
    return bundle
