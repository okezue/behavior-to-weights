...
from behavior2weights.train.inverse import(InverseTrainingConfig,load_inverse_checkpoint,train_inverse_model,)
...
from behavior2weights.zoo.micro import(InterventionSpec,MicroZooConfig,OptimizerSpec,build_micro_zoo,)
def run_smoke_pipeline(output_directory:str|Path,*,overwrite:bool=False)->dict[str,Any]:
    ...
    zoo=build_micro_zoo(zoo_config,output/"zoo",split_policy=SplitPolicy(train_fraction=0.625,validation_fraction=0.125,test_fraction=0.25),)
    ...
    trace_bundle=collect_target_traces(zoo.records,queries,adapter.load,ObservationConfig(channel=ObservationChannel.LOGITS,vocab_size=architecture.vocab_size,feature_dim=architecture.vocab_size,),CollectionConfig(batch_size=64,device="cpu",base_seed=91),output_directory=output/"traces"/"logits",)
    ...
    result=train_inverse_model(BehaviorToWeights(inverse_config),corpus,training_config,output/"inverse",tracker=tracker,)
    ...
    prediction_vector=standardizer.inverse_transform(standardized_mean.squeeze(0),corpus.role_ids)
    ...
    train_vectors=torch.stack([corpus.target_vector(index)for index in corpus.indices_for_split(Split.TRAIN)])
    mean_baseline=train_vectors.mean(dim=0)
    report={"status":"completed","zoo":manifest_summary(zoo.records),"trace_shape":list(trace_bundle.observations.shape),"inverse_parameters":sum(parameter.numel()for parameter in model.parameters()),"target_parameters":corpus.address_space.total_parameters,"training":{"steps_completed":result.steps_completed,"best_validation_nll":result.best_validation_nll,},"evaluation_target":corpus.targets[target_index].record.target_id,"weight_nrmse":normalized_rmse(prediction_vector,target_vector),"mean_checkpoint_nrmse":normalized_rmse(mean_baseline,target_vector),"functional":functional,"note":"Smoke results validate execution only; they are not powered scientific estimates.",}
