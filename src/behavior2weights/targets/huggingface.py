...
class HuggingFaceCausalLMAdapter(TargetAdapter):
    ...
    def load(self,record:TargetRecord,*,device:str|torch.device="cpu")->nn.Module:
        ...
        model=cast(Any,AutoModelForCausalLM.from_pretrained(source,revision=revision,trust_remote_code=trust_remote_code,local_files_only=self.local_files_only,torch_dtype=dtype,))
