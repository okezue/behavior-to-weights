...
class HuggingFaceCausalLMAdapter(TargetAdapter):
    """Adapter for local or Hub-compatible causal LMs.

    Remote code is disabled by default. A target manifest must opt in explicitly, and production
    runs should pin both model and code revisions after an independent security review.
    """

    family = "huggingface-causal-lm"

    def __init__(
        self, *, manifest_root: str | Path | None = None, local_files_only: bool = False
    ) -> None:
    ...
    def load(self, record: TargetRecord, *, device: str | torch.device = "cpu") -> nn.Module:
        try:
            ...
            raise RuntimeError(
                "Install behavior2weights[hf] to load Hugging Face models"
            ) from error
