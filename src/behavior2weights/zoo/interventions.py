...
def ablate_attention_head(
    ...
    *,
    ...
    return InterventionResult(
        state_dict=result,
        record=InterventionRecord(
            kind="attention_head_ablation",
            label=f"L{layer}.H{head}",
            layer=layer,
            head=head,
            metadata={"head_dim": config.head_dim},
        ),
        changed_tensors=tuple(changed),
        changed_entries=entries,
    )


def ablate_mlp_neuron(
    state_dict: Mapping[str, Tensor],
    config: MicroTransformerConfig,
    *,
    layer: int,
...
def apply_lora_edit(
    ...
    *,
    ...
    return InterventionResult(
        ...
        record=InterventionRecord(
            kind="lora_edit",
            label=f"{tensor_name}:r{rank}:s{scale:g}",
            tensor_name=tensor_name,
            rank=rank,
            scale=scale,
            seed=seed,
            metadata={"update_frobenius_norm": float(update.norm().item())},
        ),
        changed_tensors=(tensor_name,),
        changed_entries=weight.numel(),
    )


def apply_sparse_weight_edit(
    state_dict: Mapping[str, Tensor],
    *,
    tensor_name: str,
    count: int,
    ...
    return InterventionResult(
        state_dict=result,
        record=InterventionRecord(
            kind="sparse_weight_edit",
            label=f"{tensor_name}:k{count}:s{scale:g}",
            tensor_name=tensor_name,
            scale=scale,
            seed=seed,
            metadata={"count": count, "flat_indices": indices.cpu().tolist()},
        ),
        changed_tensors=(tensor_name,),
