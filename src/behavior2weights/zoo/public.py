...
def build_public_manifest(config:PublicManifestConfig,output_path:str|Path)->list[TargetRecord]:
    records:list[TargetRecord]=[]
    for model in config.models:
        architecture_id=model.architecture_id or f"hf-{stable_hash(model.model_name)}"
        lineage_id=(f"public-{stable_hash({'model': model.model_name, 'family': model.external_family})}")
