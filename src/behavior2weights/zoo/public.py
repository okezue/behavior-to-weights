...
from behavior2weights.zoo.manifest import save_manifest
...
class PublicManifestConfig:
    ...
    @classmethod
    def from_dict(cls, raw: dict[str, Any]) -> PublicManifestConfig:
...
def build_public_manifest(
    config: PublicManifestConfig, output_path: str | Path
) -> list[TargetRecord]:
    records: list[TargetRecord] = []
    for model in config.models:
        architecture_id = model.architecture_id or f"hf-{stable_hash(model.model_name)}"
        lineage_id = (
            f"public-{stable_hash({'model': model.model_name, 'family': model.external_family})}"
        )
        for revision in model.revisions:
            target_id = f"public-{stable_hash({'model': model.model_name, 'revision': revision})}"
            step = None
            ...
            records.append(
                TargetRecord(
                    target_id=target_id,
                    family_id="huggingface-causal-lm",
                    lineage_id=lineage_id,
                    architecture_id=architecture_id,
                    task_id="public-pretraining",
                    dataset_id=str(model.metadata.get("dataset_id", "unknown")),
                    seed=int(model.metadata.get("seed", 0)),
                    checkpoint_path=Path(model.model_name),
                    training_step=step,
                    split=model.split,
                    factors={
                    ...
                    metadata={
                        **model.metadata,
                        "model_name": model.model_name,
                        "revision": revision,
                        "tokenizer_name": model.tokenizer_name or model.model_name,
