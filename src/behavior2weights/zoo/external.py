...
TRANSFORMER_NFN_DATASETS={"mnist":"https://huggingface.co/datasets/anonymized-acamedia/Small-Transformer-Zoo/resolve/main/MNIST-Transformers.zip","ag_news":"https://huggingface.co/datasets/anonymized-acamedia/Small-Transformer-Zoo/resolve/main/AG-News-Transformers.zip",}
...
def download_transformer_nfn_zoo(name:str,destination:str|Path,*,expected_sha256:str|None=None,overwrite:bool=False,)->DownloadResult:
    ...
    if not archive.exists():
        with(urllib.request.urlopen(TRANSFORMER_NFN_DATASETS[name])as response,archive.open("wb")as output,):
            shutil.copyfileobj(response,output)
    digest=_sha256(archive)
    if expected_sha256 and digest!=expected_sha256:
        archive.unlink(missing_ok=True)
        raise ValueError(f"Checksum mismatch for {archive}: expected {expected_sha256}, got {digest}")
