...
def _split_counts(factor_rows:Sequence[Mapping[str,Any]],policy:SplitPolicy)->dict[str,int]:
    ...
    for factors in factor_rows:
        is_ood=any(str(factors.get(factor))in{str(value)for value in values}for factor,values in policy.ood_factor_values.items())
        if is_ood:
            ood+=1
        else:
            in_distribution+=1
...
def plan_micro_zoo(config:MicroZooConfig,*,split_policy:SplitPolicy|None=None,name:str|None=None,)->ZooProjection:
    ...
    for architecture_index,architecture in enumerate(config.architectures):
        ...
        architecture_plans.append(ArchitectureProjection(architecture_index=architecture_index,configuration=dataclasses.asdict(architecture),parameters=parameter_count,raw_parameter_bytes_fp32=parameter_count*4,lineages=lineages,checkpoint_targets=checkpoint_targets,intervention_targets=intervention_targets,total_targets=total_targets,projected_raw_checkpoint_bytes_fp32=total_targets*parameter_count*4,))
...
def plan_text_zoo(config:TextZooConfig,*,split_policy:SplitPolicy|None=None,name:str|None=None,)->ZooProjection:
    ...
    interventions_per_lineage=sum(spec.count for spec in config.interventions)
    targets_per_lineage=checkpoints_per_lineage+interventions_per_lineage
    lineages_per_architecture=(len(config.model_seeds)*len(config.data_order_seeds)*len(config.optimizers))
    architecture_plans:list[ArchitectureProjection]=[]
    factor_rows:list[dict[str,Any]]=[]
    for architecture_index,architecture in enumerate(config.architectures):
        parameter_count=micro_parameter_count(architecture)
