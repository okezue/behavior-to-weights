...
from behavior2weights.zoo.interventions import (
    ablate_attention_head,
    ablate_mlp_neuron,
    apply_lora_edit,
    apply_sparse_weight_edit,
)
...
class MicroZooConfig:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> MicroZooConfig:
        ...
        data["architectures"] = tuple(
            MicroTransformerConfig.from_dict(item) for item in data["architectures"]
        )
        data["optimizers"] = tuple(
            OptimizerSpec(**item)
            for item in data.get("optimizers", [dataclasses.asdict(OptimizerSpec())])
        )
        data["interventions"] = tuple(
            InterventionSpec(**item) for item in data.get("interventions", [])
        )
        if "tasks" in data:
...
def evaluate_loss(model: MicroTransformer, sequences: torch.Tensor, batch_size: int = 128) -> float:
    ...
    return float(
        sum(loss * weight for loss, weight in zip(losses, weights, strict=True)) / sum(weights)
    )
    ...
    for spec in specs:
        for local_index in range(spec.count):
            ...
            if spec.kind == "attention_head_ablation":
                ...
                result = apply_lora_edit(
                    base_state,
                    tensor_name=tensor_name,
                    rank=min(spec.rank, min(base_state[tensor_name].shape)),
                    scale=spec.scale,
                ...
                result = apply_sparse_weight_edit(
                    base_state,
                    tensor_name=tensor_name,
                    count=count,
                    scale=spec.scale,
            ...
            edit_id = stable_hash(
                {
                    "parent": base_record.target_id,
                    "intervention": result.record.model_dump(mode="json"),
                }
            )
            relative_path = (
                Path("checkpoints") / base_record.lineage_id / f"edit-{edit_id}.safetensors"
            )
            path, checksum = _save_target(
                edited,
                root=root,
                relative_path=relative_path,
                metadata={"target_id": edit_id, "parent_target_id": base_record.target_id},
            )
            records.append(
                base_record.model_copy(
                    update={
                        ...
                        "parent_target_id": base_record.target_id,
                        "interventions": [result.record],
                        "factors": {
                            **base_record.factors,
                            "intervention_kind": result.record.kind,
                            "intervention_label": result.record.label,
                        },
                        "metadata": {
                            **base_record.metadata,
                            "changed_tensors": list(result.changed_tensors),
...
def build_micro_zoo(
    ...
    *,
    ...
    combinations = itertools.product(
        ...
        config.tasks,
        config.model_seeds,
        config.dataset_seeds,
        config.optimizers,
    )
    for (
        architecture_index,
        architecture,
    ), task, model_seed, dataset_seed, optimizer_spec in combinations:
        ...
        batcher = SequenceBatcher(
            splits["train"], config.batch_size, shuffle=True, seed=model_seed + dataset_seed
        )
        ...
        while step <= config.train_steps:
            if step in checkpoint_steps:
                ...
                record = TargetRecord(
                    ...
                    task_id=task,
                    dataset_id=f"synthetic-{task}-seed{dataset_seed}",
                    seed=model_seed,
                    checkpoint_path=path,
                    checkpoint_sha256=checksum,
                    training_step=step,
        ...
        records.extend(
            _intervention_targets(
                ...
                root=root,
                lineage_seed=int(stable_hash(lineage_payload, length=8), 16),
            )
        )
