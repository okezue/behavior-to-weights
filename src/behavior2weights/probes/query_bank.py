...
def random_query_tensor(config: QueryBankConfig) -> Tensor:
    ...
    return torch.randint(
        config.vocab_size,
        (config.random_queries, config.seq_len),
        generator=generator,
    )
...
def natural_query_tensor(config: QueryBankConfig, task: str = "mixture") -> Tensor:
    synthetic = SyntheticDatasetConfig(
        ...
        train_examples=config.natural_queries,
...
def exhaustive_query_tensor(config: QueryBankConfig) -> Tensor:
    if config.vocab_size > config.exhaustive_max_vocab:
        raise ValueError(
            "exhaustive query enumeration disabled: vocab_size exceeds exhaustive_max_vocab"
        )
    if config.seq_len > config.exhaustive_max_length:
        raise ValueError(
            "exhaustive query enumeration disabled: seq_len exceeds exhaustive_max_length"
        )
...
def build_query_bank(
    ...
    *,
    include: Sequence[str] = ("random", "natural"),
) -> list[QueryRecord]:
    records: list[QueryRecord] = []
