...
def preparehfcausallmdataset(config:HFTextDatasetConfig,output_directory:str|Path,*,overwrite:bool=False,)->Path:
    try:
        ...
        raise RuntimeError("Install behavior2weights[hf] to prepare Hugging Face datasets")from error
    output_directory=Path(output_directory)
    manifest_path=output_directory/"dataset_manifest.json"
    if manifest_path.exists()and not overwrite:
        load_from_disk(str(output_directory/"arrow"))
        return manifest_path
    dataset=load_dataset(config.dataset_name,config.dataset_config,revision=config.revision,)
    ...
    split_names={"train":config.train_split,"validation":config.validation_split,"test":config.test_split,}
    limits={"train":config.max_train_examples,"validation":config.max_validation_examples,"test":config.max_test_examples,}
