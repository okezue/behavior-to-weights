...
def _copy_sequences(config:SyntheticDatasetConfig,count:int,seed:int)->Tensor:
    ...
    for row in prefix:
        ...
        if sequence.numel()<config.seq_len:
            padding=torch.randint(usable,(config.seq_len-sequence.numel(),),generator=generator)
...
def _induction_sequences(config:SyntheticDatasetConfig,count:int,seed:int)->Tensor:
    ...
    for row in range(count):
        ...
        second_position=int(torch.randint(first_position+2,config.seq_len-1,(1,),generator=generator).item())
