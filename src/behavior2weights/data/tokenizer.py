...
def train_byte_bpe_tokenizer(
    ...
    *,
    ...
    """Train and save a byte-level BPE tokenizer from a pinned Hugging Face dataset."""
    try:
    ...
    manifest = {
        ...
        "actual_vocab_size": len(fast),
        "tokenizer_files": sorted(path.name for path in output.iterdir() if path.is_file()),
    }
