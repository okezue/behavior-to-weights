...
class PowerResult:
    ...
    power:float
    simulations:int
def analytic_paired_sample_size(standardized_effect:float,*,alpha:float=0.05,power:float=0.80,two_sided:bool=True,)->int:
...
def simulate_clustered_power(*,lineages:int,checkpoints_per_lineage:int,standardized_effect:float,intraclass_correlation:float=0.5,alpha:float=0.05,simulations:int=5_000,seed:int=0,)->PowerResult:
...
def find_minimum_lineages(*,standardized_effect:float,checkpoints_per_lineage:int=1,intraclass_correlation:float=0.5,target_power:float=0.8,alpha:float=0.05,min_lineages:int=8,max_lineages:int=512,simulations:int=2_000,seed:int=0,)->PowerResult:
    last:PowerResult|None=None
    for lineages in range(min_lineages,max_lineages+1,4):
        last=simulate_clustered_power(lineages=lineages,checkpoints_per_lineage=checkpoints_per_lineage,standardized_effect=standardized_effect,intraclass_correlation=intraclass_correlation,alpha=alpha,simulations=simulations,seed=seed+lineages,)
