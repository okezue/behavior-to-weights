...
def _paired_cluster_values(
    ...
    *,
    ...
    grouped = (
        subset.groupby([cluster_column, method_column], as_index=False)[value_column]
        .mean()
        .pivot(index=cluster_column, columns=method_column, values=value_column)
    )
...
def paired_cluster_bootstrap(
    ...
    *,
    ...
    differences = _paired_cluster_values(
        frame,
        method_a=method_a,
        method_b=method_b,
        metric=metric,
    ...
    draws = generator.choice(differences, size=(resamples, len(differences)), replace=True).mean(
        axis=1
    )
    alpha = 1 - confidence
    ...
    return Estimate(
        ...
        method="paired lineage-cluster bootstrap",
    )
...
def one_sample_summary(values: Sequence[float], confidence: float = 0.95) -> Estimate:
    ...
    return Estimate(
        estimate=float(mean),
        lower=float(mean - quantile * sem),
        upper=float(mean + quantile * sem),
