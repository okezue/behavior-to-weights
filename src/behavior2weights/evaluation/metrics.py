...
def functionalmetrics(prediction_model:nn.Module,target_model:nn.Module,input_ids:Tensor,*,batch_size:int=128,device:str|torch.device="cpu",)->dict[str,float]:
    ...
    for start in range(0,len(input_ids),batch_size):
        ...
        kl_sum+=float(F.kl_div(prediction_log_probs,target_probs,reduction="batchmean").item()*batch_count)
        reverse_kl_sum+=float(F.kl_div(target_log_probs,prediction_probs,reduction="batchmean").item()*batch_count)
        agreement_sum+=float((prediction_logits.argmax(-1)==target_logits.argmax(-1)).float().sum().item())
...
def localizationmetrics(scores:Tensor,correct_indices:Tensor)->dict[str,float]:
    ...
    best_rank=min(ranks)
    return{"reciprocal_rank":1.0/best_rank,"top1":float(best_rank<=1),"top3":float(best_rank<=3),"top5":float(best_rank<=5),"best_rank":float(best_rank),}
