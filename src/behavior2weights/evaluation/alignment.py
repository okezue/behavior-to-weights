...
def attention_head_features(
    state_dict: Mapping[str, Tensor], config: MicroTransformerConfig, layer: int
) -> Tensor:
    ...
    for head in range(config.n_heads):
        ...
        parts = [
            state_dict[f"{prefix}.q_proj.weight"][start:end, :],
            state_dict[f"{prefix}.k_proj.weight"][start:end, :],
            state_dict[f"{prefix}.v_proj.weight"][start:end, :],
            state_dict[f"{prefix}.o_proj.weight"][:, start:end],
        ]
...
def _assignment(reference: Tensor, candidate: Tensor) -> Tensor:
    ...
    norm_cost = (
        reference.float().norm(dim=1)[:, None] - candidate.float().norm(dim=1)[None, :]
    ).abs() / reference.float().norm(dim=1).median().clamp_min(1e-8)
...
def align_micro_state_dict(
    candidate: Mapping[str, Tensor],
    reference: Mapping[str, Tensor],
    config: MicroTransformerConfig,
) -> tuple[OrderedDict[str, Tensor], dict[str, list[list[int]] | list[int]]]:
    ...
    for layer in range(config.n_layers):
        head_permutation = _assignment(
            attention_head_features(reference, config, layer),
            attention_head_features(aligned, config, layer),
        )
        ...
        neuron_permutation = _assignment(
            mlp_neuron_features(reference, layer),
            mlp_neuron_features(aligned, layer),
        )
