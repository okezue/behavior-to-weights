...
def evaluate_property_classifier(
    *,
    ...
    tier: str = "tier2",
    run_id: str = "property-evaluation",
    replicate: int = 0,
    seed: int = 20260621,
    device: str = "cpu",
) -> list[ResultRecord]:
    ...
    policy_orders = {
        policy: training_population_order(
            traces.observations[training_rows],
            training_lineages,
            policy=policy,
            seed=seed + policy_index * 1_000_003,
        ).indices
        ...
        for budget in budgets:
            ...
            for name, values in logits.items():
                ...
                for row, target_index in enumerate(indices):
                    ...
                    metadata = {
                        "property": name,
                        "true_label": vocabulary.decode(name, int(labels[row])),
                        "predicted_label": vocabulary.decode(name, int(predictions[row])),
                        "unknown_target": bool(labels[row] == 0),
                        "query_index_hash": query_hash,
                        "query_policy_fit_split": "train",
                    }
                    entropy = -(
                        probabilities[row] * probabilities[row].clamp_min(1e-12).log()
                    ).sum()
                    metrics = {
                        f"property_{name}_accuracy": float(predictions[row] == labels[row]),
                        f"property_{name}_nll": float(losses[row]),
                        f"property_{name}_confidence": float(probabilities[row].max()),
                        f"property_{name}_entropy": float(
                            entropy / entropy_denominator if entropy_denominator > 0 else 0.0
                        ),
                    }
                    for metric, value in metrics.items():
                        output.append(
                            ResultRecord(
                                ...
                                split=record.split or Split.TEST,
                                tier=tier,
                                method="trace_property_classifier",
