...
class AimTracker(ExperimentTracker):
    """Thin adapter for the pinned Aim 3.x SDK and local or remote repositories."""
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "Aim 3.x is not installed. Install behavior2weights[aim], or select backend=jsonl."
            ) from error
        self.run = run_class(
            repo=repo,
        ...
        for tag in tags:
            ...
            if callable(add_tag):
                add_tag(str(tag))
            else:  # Defensive fallback for older 3.x patch releases.
