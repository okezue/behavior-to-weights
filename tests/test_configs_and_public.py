...
def test_micro_config_expands_integer_ranges()->None:
    config=MicroZooConfig.from_dict({"architectures":[{"vocab_size":16,"max_seq_len":8,"d_model":8,"n_heads":2,"n_layers":1,"d_ff":12,}],"model_seeds":{"start":2,"stop":8,"step":2},"dataset_seeds":[1],"train_steps":4,"checkpoint_steps":[0,4],})
    assert config.model_seeds==(2,4,6)
def test_public_manifest_expands_revisions(tmp_path)->None:
    config=PublicManifestConfig.from_dict({"models":[{"model_name":"example/model","revisions":["step0","step16"],"external_family":"example","split":"ood",}]})
...
def test_text_zoo_config_expands_ranges_and_rejects_unknown_fields()->None:
    ...
    config=TextZooConfig.from_dict({"architectures":[{"vocab_size":64,"max_seq_len":16,"d_model":16,"n_heads":2,"n_layers":1,"d_ff":32,}],"model_seeds":{"start":10,"stop":14,"step":2},"data_order_seeds":[3],"train_steps":4,"checkpoint_steps":[0,4],})
    ...
    with pytest.raises(ValueError,match="Unknown TextZooConfig fields"):
        TextZooConfig.from_dict({"architectures":[{"vocab_size":64,"max_seq_len":16,"d_model":16,"n_heads":2,"n_layers":1,"d_ff":32,}],"train_steps":0,"checkpoint_steps":[0],"not_a_field":True,})
def test_tokenizer_training_config_normalizes_special_tokens()->None:
    ...
    config=TokenizerTrainingConfig.from_dict({"dataset_name":"example/corpus","special_tokens":["<pad>","<unk>","<eos>"],})
