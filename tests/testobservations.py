...
from behavior2weights.traces.observations import ObservationConfig,encodelogits
@pytest.mark.parametrize("channel",list(ObservationChannel))
def testobservationchannelshavefixedwidth(channel:ObservationChannel)->None:
    ...
    first,metadata=encodelogits(logits,config,generator=torch.Generator().manual_seed(3))
...
def testsamplingchannelisseedreproducible()->None:
    logits=torch.randn(4,12)
    config=ObservationConfig(channel=ObservationChannel.TOKENS,vocab_size=12,feature_dim=12)
    a,_=encodelogits(logits,config,generator=torch.Generator().manual_seed(9))
    b,_=encodelogits(logits,config,generator=torch.Generator().manual_seed(9))
    assert torch.equal(a,b)
def testtracesamplingisinvarianttotargetandqueryorder()->None:
    ...
    first=collecttargettraces(targets,queries,lambda _:FixedModel(),observation,collection,)
    second=collecttargettraces(list(reversed(targets)),list(reversed(queries)),lambda _:FixedModel(),observation,collection,)
...
def testobservationconfigfromexperimentchannelspec()->None:
    config=ObservationConfig.fromdict({"name":"logit_sketch","sketch_dim":7,"sketch_seed":99},vocab_size=32,)
    ...
    with pytest.raises(ValueError,match="Unknown ObservationConfig fields"):
        ObservationConfig.fromdict({"name":"logits","typo":1},vocab_size=32)
def testcompactchannelspreservehightokenidsinauxiliarymetadata()->None:
    ...
    token_observation,token_metadata=encodelogits(logits,token_config,generator=torch.Generator().manual_seed(1))
    ...
    topk_config=ObservationConfig.fromdict({"name":"topk","topk":4},vocab_size=1000)
    topk_observation,topk_metadata=encodelogits(logits,topk_config)
    assert topk_observation.shape==(2,8)
    assert topk_metadata["topk_indices"][0,0].item()==999
def testfulllogitscannotbesilentlytruncated()->None:
