...
import json
import torch
from behavior2weights.analytic.softmax import(LinearSoftmaxExperimentConfig,canonicalsoftmaxparameters,probabilitiesfromcanonical,recoverlinearsoftmax,runlinearsoftmaxexperiment,)
def testfullrankprobabilityqueriesrecoversoftmaxorbitexactly()->None:
    ...
    beta=canonicalsoftmaxparameters(weight,bias)
    ...
    recovery=recoverlinearsoftmax(design,probabilities,observation_channel="probabilities")
...
def testrankdeficientqueriesreportunobservabledimensions()->None:
...
def testtier0runnerwritesstrictjsonartifacts(tmp_path)->None:
    summary=runlinearsoftmaxexperiment(LinearSoftmaxExperimentConfig(input_dims=(2,),class_counts=(3,),query_counts=(2,3),query_strategies=("basis",),observation_channels=("probabilities",),systems_per_cell=2,holdout_queries=8,),tmp_path,)
