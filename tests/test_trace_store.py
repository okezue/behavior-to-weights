...
def test_trace_store_roundtrips_auxiliary_outputs(tmp_path) -> None:
    ...
    bundle = TraceBundle(
        ...
        input_ids=torch.arange(12).reshape(3, 4),
        observations=torch.randn(2, 3, 5),
        channel=ObservationChannel.TOPK,
        feature_dim=5,
        auxiliary={
            "topk_indices": torch.randint(0, 5, (2, 3, 2)),
            "topk_values": torch.randn(2, 3, 2),
        },
    )
    save_trace_bundle(bundle, tmp_path)
    loaded = load_trace_bundle(tmp_path)
    ...
    assert torch.equal(
        loaded.subset_targets([1]).auxiliary["topk_values"],
        bundle.auxiliary["topk_values"][1:2],
    )
