...
import json
import torch
from behavior2weights.analytic.softmax import(LinearSoftmaxExperimentConfig,canonical_softmax_parameters,probabilities_from_canonical,recover_linear_softmax,run_linear_softmax_experiment,)
...
def test_tier0_runner_writes_strict_json_artifacts(tmp_path)->None:
    summary=run_linear_softmax_experiment(LinearSoftmaxExperimentConfig(input_dims=(2,),class_counts=(3,),query_counts=(2,3),query_strategies=("basis",),observation_channels=("probabilities",),systems_per_cell=2,holdout_queries=8,),tmp_path,)
