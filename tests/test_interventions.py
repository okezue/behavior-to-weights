...
def test_controlled_interventions_report_changed_coordinates() -> None:
    ...
    lora = apply_lora_edit(
        state, tensor_name="blocks.0.attn.o_proj.weight", rank=2, scale=0.1, seed=5
    )
    ...
    assert not torch.equal(
        head.state_dict["blocks.0.attn.o_proj.weight"], state["blocks.0.attn.o_proj.weight"]
    )
