...
def test_trace_sampling_is_invariant_to_target_and_query_order() -> None:
    ...
    observation = ObservationConfig(
        channel=ObservationChannel.SAMPLE_HISTOGRAM,
        vocab_size=8,
        feature_dim=8,
        sample_count=17,
    )
    collection = CollectionConfig(base_seed=19)
    first = collect_target_traces(
        targets,
        queries,
        lambda _: FixedModel(),
        observation,
    ...
    for target_id in first.target_ids:
        ...
        for query_id in first.query_ids:
            ...
            assert torch.equal(
                first.observations[first_target, first_query],
                second.observations[second_target, second_query],
            )


def test_observation_config_from_experiment_channel_spec() -> None:
    config = ObservationConfig.from_dict(
        {"name": "logit_sketch", "sketch_dim": 7, "sketch_seed": 99},
        vocab_size=32,
    )
...
def test_compact_channels_preserve_high_token_ids_in_auxiliary_metadata() -> None:
    ...
    logits[:, 999] = 20.0
    token_config = ObservationConfig(
        channel=ObservationChannel.TOKENS,
        vocab_size=1000,
        feature_dim=32,
    )
    token_observation, token_metadata = encode_logits(
        logits, token_config, generator=torch.Generator().manual_seed(1)
    )
